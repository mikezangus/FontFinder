...
def load_test_character():
    ...
    image_files = [file for file in test_character_path.glob("*.png")]
    test_character_file = random.choice(image_files)
    image = Image.open(test_character_file).convert("L")
...
def main():
    ...
    for character in characters:
        for character_type in character_types:
            ...
            with torch.no_grad():
                other_output = other_model(image_tensor)
            
            predicted_value = torch.argmax(other_output.data, 1).item()
            certainty = torch.softmax(other_output.data, 1)[0][predicted_value].item()
            prediction_scores.append((character, character_type, certainty))
    closest_prediction = sorted(prediction_scores, key = lambda x: x[2], reverse = False)[:3]
    for character, character_type, certainty in closest_prediction:
        print(f"Character: {character} {character_type}: Certainty: {certainty:.2f}")
